...
## CHANGE THESE
export MAPR_HOST_IP=your.server.ip
...
sudo groupadd -g ${MAPR_GID} ${MAPR_GROUP} && sudo useradd -m -u ${MAPR_UID} -g ${MAPR_GID} -d ${MAPR_USER_HOME} -s /bin/bash ${MAPR_USER} \
  && sudo usermod -a -G sudo ${MAPR_GROUP}
...
# Spark and Delta Lake - as per https://github.com/fbercken/fingrid

## Install livy and airflow on the server
