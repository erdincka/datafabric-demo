...
export MAPR_HOST=local.demo.df 
export MAPR_CLUSTER=local.demo.df
echo "${MAPR_HOST}" | sudo tee /etc/hostname 

IP=$(/sbin/ip -o -4 addr list enp0s7 | awk '{print $4}' | cut -d/ -f1)
HOSTNAME=$(hostname -f)
...
sudo /opt/mapr/server/configure.sh -N ${MAPR_HOST} -C ${MAPR_HOST}:7222 -Z ${MAPR_HOST} -u ${MAPR_USER} -g ${MAPR_GROUP} -genkeys
